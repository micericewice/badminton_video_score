...
def parse_csv(csv_file):
   ...
   with open(csv_file, 'r', encoding='utf-8') as file:
     ...
     for row in reader:
       row_count += 1
       if int(row[0]) < 0: raise Exception(f"timestamp (m) is invalid at row {row_count}")
       if int(row[1]) < 0 or int(row[1]) >= 60: raise Exception(f"timestamp (s) is invalid at row {row_count}")
       ...
       if timestamp_sec<=pre_score_sec_p1 or timestamp_sec<=pre_score_sec_p2 or timestamp_sec<=pre_serve_indication_sec:
         raise Exception(f"timestamp (total) is invalid at row {row_count}")
       ret['serve_indicator'] += [{
...
def parse_xlsx(xlsx_file):
   ...
   for row in rows[2:]:
      ...
      if int(row[0]) < 0:
         raise Exception(f"timestamp (m) is invalid at row {row_count}")
      if int(row[1]) < 0 or int(row[1]) >= 60:
         raise Exception(f"timestamp (s) is invalid at row {row_count}")

      timestamp_sec = int(row[0]) * 60 + int(row[1])

      if (timestamp_sec <= pre_score_sec_p1 or
         timestamp_sec <= pre_score_sec_p2 or
         timestamp_sec <= pre_serve_indication_sec):
         raise Exception(f"timestamp (total) is invalid at row {row_count}")
      ...
      cell_p1 = str(row[2]).strip() if row[2] is not None else ""
      cell_p2 = str(row[3]).strip() if row[3] is not None else ""

      if cell_p1.startswith(("x", "X")):  # p1 scored
         ret['pscore1'] += [{
            'start': pre_score_sec_p1,
            'val': score_p1,
            'duration': (timestamp_sec - pre_score_sec_p1),
      ...
      elif cell_p2.startswith(("x", "X")):  # p2 scored
         ret['pscore2'] += [{
            'start': pre_score_sec_p2,
            'val': score_p2,
            'duration': (timestamp_sec - pre_score_sec_p2),
